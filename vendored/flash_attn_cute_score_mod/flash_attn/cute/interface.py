...
def to_cute_tensor(t, assumed_align=16, leading_dim=-1, fully_dynamic=False):
    ...
    try:
        ...
        # Older nvidia-cutlass-dsl builds don't accept enable_tvm_ffi.
...
def _flash_attn_bwd(
    ...
    if compile_key not in _flash_attn_bwd.compile_cache:
        ...
        if compute_capability == 9:
            ...
            if FlashAttentionBackwardSm100 is None:
                raise NotImplementedError(
                    "FlashAttentionBackwardSm100 is unavailable in this environment "
                    "(likely due to an older nvidia-cutlass-dsl). Forward-only FA4/CuTe is supported."
                )
