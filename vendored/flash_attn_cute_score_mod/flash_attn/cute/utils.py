...
_TENSORSSA_HAS_BROADCAST_TO = hasattr(cute.TensorSSA, "broadcast_to")

if _TENSORSSA_HAS_BROADCAST_TO:

    @cute.jit
    def scalar_to_ssa_vec(a: cute.Numeric, dtype, n: cutlass.Constexpr[int]) -> cute.TensorSSA:
        """Convert a scalar to a cute TensorSSA of shape (n,) and given dtype."""
        return scalar_to_ssa(a, dtype).broadcast_to((n,))
    ...
    def scalar_to_ssa_vec(a: cute.Numeric, dtype, n: cutlass.Constexpr[int]) -> cute.TensorSSA:
        """Convert a scalar to a cute TensorSSA of shape (n,) and given dtype."""
        vec = cute.make_fragment(n, dtype)
