...
- Other major win (B300): avoid the slow Conv3d patch-embedding path by using an equivalent per-frame Conv2d fastpath when `patch_size[0]==1`; this eliminates a large `aten::copy_`/`aten::fill_` storm from `slow_conv_dilated3d`.
- End-to-end: ~15 FPS → ~20 FPS on B200 and 8.8 FPS → 15 FPS on B300 (cu130) in daydream-style runs; the `scripts/profile_krea_pipeline_blocks.py` benchmark now reaches **~25 FPS** on B300 with `--compile + fp8 + SCOPE_KV_BIAS_BACKEND=fa4` at `320x576`.
- Compile/FP8 note (B300): `torch.compile` is high-upside but has known SM103 footguns (cudagraph “output overwritten” in `reduce-overhead`). TorchAO FP8 + compile is **blocked upstream** by `torchao.quantization.Float8Tensor` missing `aten.as_strided.default`, but is unblocked locally via a PerTensor-only monkeypatch (`src/scope/core/compat/torchao_float8_as_strided.py`; disable with `SCOPE_TORCHAO_PATCH_FLOAT8_AS_STRIDED=0`). See [`torchao-as-strided-dispatch.md`](../../issues/torchao-as-strided-dispatch.md).
